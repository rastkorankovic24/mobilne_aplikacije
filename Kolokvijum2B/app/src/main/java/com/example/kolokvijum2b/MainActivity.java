...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    private static final String PREFS_NAME = "Kolokvijum2BPrefs";
    private static final String PREFS_KEY_TEKST = "tekst";
    private static final String CHANNEL_ID = "users_channel";
    private static final String FILE_PROVIDER_AUTHORITY = "com.example.kolokvijum2b.fileprovider";
    ...
    private final ActivityResultLauncher<String> contactsPermissionLauncher =
            registerForActivityResult(new ActivityResultContracts.RequestPermission(), granted -> {
                if (granted) {
                    showFirstContactEmailInTextView();
                } else {
    ...
    private void setupListeners() {
        ...
        switchUsers.setOnCheckedChangeListener(this::onSwitchChanged);
        buttonAction.setOnClickListener(v -> deleteLastUserAndNotify());
    }

    // CheckBox mora biti čekiran da bi Switch ON radio
    ...
    // Kamera + žiroskop Toast posle slike
    ...
    // Switch ON: prvi put 8 korisnika, sledeći put ime prvog u bazi
    private void handleSwitchOn() {
        ...
        if (dbHelper.getUserCount() == 0) {
            Toast.makeText(this, R.string.users_loading, Toast.LENGTH_SHORT).show();
            fetchAndSaveUsers();
        } else {
            String name = dbHelper.getFirstUserName();
            if (name != null) {
        ...
        userService.getUsers().enqueue(new Callback<List<User>>() {
            ...
            public void onResponse(@NonNull Call<List<User>> call, @NonNull Response<List<User>> response) {
                if (response.isSuccessful() && response.body() != null && !response.body().isEmpty()) {
                    ...
                    int count = Math.min(users.size(), 8);
                    dbHelper.insertUsers(users.subList(0, count));
                    Toast.makeText(MainActivity.this,
                            getString(R.string.users_loaded, count),
                            Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(MainActivity.this, R.string.users_fetch_error, Toast.LENGTH_LONG).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<List<User>> call, @NonNull Throwable t) {
                Toast.makeText(MainActivity.this,
                        getString(R.string.users_fetch_error_detail, t.getMessage()),
                        Toast.LENGTH_LONG).show();
            }
        });
    }

    // Switch OFF: sačuvaj tekst u prefs, prikaži email prvog kontakta
    private void handleSwitchOff() {
        SharedPreferences prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
        prefs.edit().putString(PREFS_KEY_TEKST, textViewLocation.getText().toString()).apply();
        Toast.makeText(this, R.string.text_saved_prefs, Toast.LENGTH_SHORT).show();
    ...
    private void showFirstContactEmailInTextView() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            textViewLocation.setText(email);
            cursor.close();
        } else {
            textViewLocation.setText(R.string.no_email_found);
    ...
    // Dugme: obriši poslednjeg korisnika, prazna baza -> notifikacija
    private void deleteLastUserAndNotify() {
        if (dbHelper.getUserCount() == 0) {
            sendEmptyDbNotification();
            Toast.makeText(this, R.string.empty_db_notification, Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(
                    CHANNEL_ID,
                    getString(R.string.channel_users),
    ...
    private void sendEmptyDbNotification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS)
                    != PackageManager.PERMISSION_GRANTED) {
                return;
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, CHANNEL_ID)
                ...
                .setContentText(getString(R.string.empty_db_notification))
    ...
    protected void onResume() {
        ...
        if (switchUsers.isChecked() && hasLocationPermission()) {
            loadLocation();
        }
    }
